...
defmodule Ex02 do
  ...
  def new_counter(initial_value \\ 0) do
    {:ok, myagent} = Agent.start_link(fn -> initial_value end)
    myagent
  end

  def next_value(agent) do
    nextvalue = Agent.get_and_update(agent, fn total -> {total, total + 1} end)
    nextvalue
  end
  ...
  def new_global_counter(initial_value \\ 0) do
    ...
    {:ok, myagent} = Agent.start_link(fn -> initial_value end, [name: @global_name])
    end

  def global_next_value do
    nextvalue = Agent.get_and_update(:my_global_agent, fn total -> {total, total + 1} end)
    nextvalue
